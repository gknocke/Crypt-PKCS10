# This Makefile is for the Crypt::PKCS10 extension to perl.
#
...
#     ABSTRACT => q[Parses PKCS #10 requests to perl data structure.]
#     AUTHOR => [q[Gideon Knocke <gknocke@cpan.org>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => { ExtUtils::MakeMaker=>q[6.30] }
#     DISTNAME => q[Crypt-PKCS10]
#     EXE_FILES => []
#     LICENSE => q[perl]
#     NAME => q[Crypt::PKCS10]
#     PREREQ_PM => { Convert::ASN1=>q[0.27] }
#     TEST_REQUIRES => {  }
#     VERSION => q[0.01]
#     test => { TESTS=>q[] }
...
TO_INST_PM = lib/Crypt/PKCS10.pm

PM_TO_BLIB = lib/Crypt/PKCS10.pm \
...
PKCS10.def: Makefile.PL
	$(PERLRUN) -MExtUtils::Mksymlists \
     -e "Mksymlists('NAME'=>\"Crypt::PKCS10\", 'DLBASE' => '$(BASEEXT)', 'DL_FUNCS' => {  }, 'FUNCLIST' => [], 'IMPORTS' => {  }, 'DL_VARS' => []);"
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) "abstract: 'Parses PKCS #10 requests to perl data structure.'" >> META_new.yml
	$(NOECHO) $(ECHO) author: >> META_new.yml
	$(NOECHO) $(ECHO) "  - 'Gideon Knocke <gknocke@cpan.org>'" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "name: Crypt-PKCS10" >> META_new.yml
	$(NOECHO) $(ECHO) no_index: >> META_new.yml
	$(NOECHO) $(ECHO) "  directory:" >> META_new.yml
	$(NOECHO) $(ECHO) "    - t" >> META_new.yml
	$(NOECHO) $(ECHO) "    - inc" >> META_new.yml
	$(NOECHO) $(ECHO) requires: >> META_new.yml
	$(NOECHO) $(ECHO) "  Convert::ASN1: 0.27" >> META_new.yml
	$(NOECHO) $(ECHO) "version: 0.01" >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) {{ > META_new.json
	$(NOECHO) $(ECHO) "   \"abstract\" : \"Parses PKCS #10 requests to perl data structure.\"," >> META_new.json
	$(NOECHO) $(ECHO) "   \"author\" : [" >> META_new.json
	$(NOECHO) $(ECHO) "      \"Gideon Knocke ^<gknocke^@cpan.org^>\"" >> META_new.json
	$(NOECHO) $(ECHO) "   ]," >> META_new.json
	...
	$(NOECHO) $(ECHO) "   }}," >> META_new.json
	$(NOECHO) $(ECHO) "   \"name\" : \"Crypt-PKCS10\"," >> META_new.json
	...
	$(NOECHO) $(ECHO) "         \"requires\" : {{" >> META_new.json
	$(NOECHO) $(ECHO) "            \"Convert::ASN1\" : \"0.27\"" >> META_new.json
	$(NOECHO) $(ECHO) "         }}" >> META_new.json
	$(NOECHO) $(ECHO) "      }}" >> META_new.json
	$(NOECHO) $(ECHO) "   }}," >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) "    <ABSTRACT>Parses PKCS #10 requests to perl data structure.</ABSTRACT>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "    <AUTHOR>Gideon Knocke &lt;gknocke@cpan.org&gt;</AUTHOR>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "    <IMPLEMENTATION>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Convert::ASN1\" VERSION=\"0.27\" />" >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e "pm_to_blib({{@ARGV}}, '$(INST_LIB)\auto', q[$(PM_FILTER)], '$(PERM_DIR)')" -- \
	  lib/Crypt/PKCS10.pm blib\lib\Crypt\PKCS10.pm 
